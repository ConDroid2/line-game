...
public class ColorAlphaModulator : MonoBehaviour
{
    ...
    public void ModulateAlpha()
    {
        ...
        Debug.Log($"NewCallNumber = {this.currentCallNumber+1}\n");

        int newCallNumber = this.currentCallNumber + 1 > this.callsToIgnore ? 0 : this.currentCallNumber + 1;

        Debug.Log($"NewCallNumber after potential reset is = {newCallNumber}\n");

        if (newCallNumber == 0) // only do the alpha setting and stuff when the call is set to zero.
        {
        ...
        this.currentCallNumber = newCallNumber;
    }
